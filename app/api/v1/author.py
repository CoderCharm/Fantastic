...
def author_info():
	...
	data = {"code": 200,
	        "count": len(data_list),
	        "account": {
		        "avatar": info_res[0].t_author_img,
		        "description": "",
		        "id": author_id,
		        "nickname": info_res[0].t_author,
	        },
	        "data": data_list,
	        "msg": "success",
	        "page": page,
...
# 热门账号
@api.route('/get/popular', methods=["GET"])
def author_popular():
...
# 作者导入接口
@api.route('/autor/add', methods=["GET"])
def author_add():
	# 查询所有的作者id，作者名字，作者来源
	info_all = FanTask.query.with_entities(FanTask.t_author, FanTask.t_author_id, FanTask.task_platform).all()
	print(info_all)
	info_all = set(info_all)
	print(info_all)
	for info in info_all:
		try:
			author = FanTaskAuthor(t_author=info[0], t_author_id=info[1], channel_id=info[2])
